...
class FluidSimulationEngine {
  ...
  setupEventListeners() {
    this.onMouseMove = (e: MouseEvent) => {
      // 使用全局鼠标位置，即使canvas设置了pointer-events-none
      this.lastMouse.copy(this.mouse);
      this.mouse.x = e.clientX / window.innerWidth;
      this.mouse.y = 1 - e.clientY / window.innerHeight;
      
      this.mouseForce.x = (this.mouse.x - this.lastMouse.x) * 5;
      this.mouseForce.y = (this.mouse.y - this.lastMouse.y) * 5;
    };
    ...
    // 监听全局事件而不是canvas事件，这样即使canvas设置了pointer-events-none也能工作
  ...
  applyViscosity() {
    ...
    const finalViscous = 20 % 2 === 0 ? this.fbos.vel_viscous0 : this.fbos.vel_viscous1;
  ...
  applyPressure() {
    ...
    const finalPressure = 20 % 2 === 0 ? this.fbos.pressure_0 : this.fbos.pressure_1;
  ...
  renderToFBO(material: THREE.Material, fbo: THREE.WebGLRenderTarget) {
  ...
  animate = () => {
    ...
    this.mouseForce.multiplyScalar(0.8);
  }

  dispose() {
    this.isAnimating = false;
    
    // Remove event listeners - 更新为document级别的监听器
...
const AudioWaveAnimation = () => {
  ...
  const simulationRef = useRef<FluidSimulationEngine | null>(null);
  ...
  useEffect(() => {
    ...
    if (!audioCanvas || !fluidCanvas) return;
    ...
    // 初始化流体模拟
    simulationRef.current = new FluidSimulationEngine(fluidCanvas);
    ...
    const animate = () => {
      ...
      calculateCurrentStage(audioCanvas, progress);
      interpolateToTarget();
      ...
      for (const bar of visibleBars) {
        ...
        // 使用我们的自定义函数绘制圆角矩形
        drawRoundedRect(-bar.width / 2, -bar.height / 2, bar.width, bar.height, cornerRadius);
        ctx.fill();
